...
async def test_add_note(tmp_path):
    ...
    note = await km.add_note(
        NoteCreate(title="Python装饰器", content="装饰器是语法糖", tags=["Python"])
    )
...
async def test_search_notes(tmp_path):
    ...
    await km.add_note(
        NoteCreate(title="异步编程", content="async await 用法", tags=["Python"])
    )
...
async def test_search_notes_by_tag(tmp_path):
    ...
    await km.add_note(
        NoteCreate(
            title="笔记A",
            content="Python装饰器是一种语法糖，用于修改函数行为",
            tags=["Python", "算法"],
        )
    )
    await km.add_note(
        NoteCreate(
            title="笔记B",
            content="MySQL索引优化可以显著提升数据库查询性能",
            tags=["数据库"],
        )
    )
...
async def test_get_all_tags(tmp_path):
    ...
    await km.add_note(
        NoteCreate(title="笔记1", content="内容", tags=["Python", "算法"])
    )
    await km.add_note(
        NoteCreate(title="笔记2", content="内容", tags=["Python", "数据库"])
    )
...
async def test_add_note_auto_tags(tmp_path):
    ...
    note = await km.add_note(
        NoteCreate(title="动态规划", content="背包问题是经典的动态规划算法题")
    )
